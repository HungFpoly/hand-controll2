...
function init_handTracking() {
  ...
  let idealWidth = window.innerWidth * dpr,
    idealHeight = window.innerHeight * dpr;
  const maxDim = Math.max(idealWidth, idealHeight);
  const scale = Math.min(1, 1024 / maxDim);
  (idealWidth = Math.round(idealWidth * scale)),
    (idealHeight = Math.round(idealHeight * scale));
  ...
  HandTrackerNavigationHelper.init({
    ...
    NNsPaths: [NNPath + "NN_NAV_RP_9.json"],
    threshold: 0.95,
    videoSettings: {
    ...
    GLSLPointerCursorColor:
      "color = mix(vec3(0.0, 0.8, 1.0), vec3(1.0, 0.0, 0.0), downFactor);",
